...
def render_general_performance_table_op(
    ...
    # ===== Продажи =====
    t.genRowTitle("Продажи")

    # Доля дозвона (%)
    t.genCellTD("Доля дозвона от всех попыток соединения", "font-weight: bold;")
    t.genCellTD("%")
    t.multiGenCellTD(_values_by_dates(general_performance_data.get("shareOfConnection"), dates, "date", "value", 1))
    t.genCellTD(_fmt_num(general_performance_data.get("averageShareOfConnection"), 1), "font-weight: bold;")
    t.genRow()

    # Количество подключений (ед.)
    t.genCellTD("Количество подключений", "font-weight: bold;")
    t.genCellTD("ед.")
    t.multiGenCellTD(_values_by_dates(general_performance_data.get("connections"), dates, "DAY", "CONNECTIONS", 0))
    t.genCellTD(_fmt_num(general_performance_data.get("averageConnections"), 0), "font-weight: bold;")
    t.genRow()

    # Конверсия подключений (%)
    t.genCellTD("Конверсия подключений", "font-weight: bold;")
    t.genCellTD("%")
    t.multiGenCellTD(_values_by_dates(general_performance_data.get("conversion"), dates, "DAY", "value", 1))
    t.genCellTD(_fmt_num(general_performance_data.get("averageConversion"), 1), "font-weight: bold;")
    t.genRow()

    # AHT (сек.)
    t.genCellTD("Average Handling Time", "font-weight: bold;")
    t.genCellTD("сек.")
    t.multiGenCellTD(_values_by_dates(general_performance_data.get("aht"), dates, "date", "value", 1))
    t.genCellTD(_fmt_num(general_performance_data.get("averageAHT"), 1), "font-weight: bold;")
    t.genRow()

    # ART (сек.)
    t.genCellTD("Average Ring Time", "font-weight: bold;")
    t.genCellTD("сек.")
    t.multiGenCellTD(_values_by_dates(general_performance_data.get("art"), dates, "date", "value", 1))
    t.genCellTD(_fmt_num(general_performance_data.get("averageART"), 1), "font-weight: bold;")
    t.genRow()
...
@op(
    ...
    description=(
        ...
        "Окраска: глобальные min/max для всех колонок, КРОМЕ 'всего'."
    ),
)
def render_day_profile_op(context, detailed_data: List[Dict[str, Any]]) -> str:
    ...
    # 2) список колонок (new_name) в порядке появления
    columns: List[str] = []
    seen = set()
    ...
    # 3) пивот по old_name×new_name, считаем суммы
    ...
    # 4) сортировка строк по 'всего' убыв.
    pivot_data.sort(key=lambda r: r.get("всего", 0), reverse=True)

    # 5) глобальные min/max ТОЛЬКО по обычным колонкам (без 'всего')
    non_total_vals: List[int] = []
    for r in pivot_data:
        for c in columns:  # внимание: без 'всего'
            v = r.get(c, 0)
            try:
                non_total_vals.append(int(v))
            except Exception:
                pass
    if non_total_vals:
        global_min = min(non_total_vals)
        global_max = max(non_total_vals)
    else:
        global_min = global_max = 0

    # (для логов) отдельные min/max по колонке 'всего'
    total_vals = [int(r.get("всего", 0) or 0) for r in pivot_data] or [0]
    total_min, total_max = min(total_vals), max(total_vals)
    context.log.info(f"Paint bounds: non-total min={global_min}, max={global_max}; total min={total_min}, max={total_max}")

    # 6) поля для BasicTable
    fields: List[Dict[str, Any]] = [{"field": "old_name", "title": "Старый ТП"}]

    # обычные колонки — красим по ГЛОБАЛЬНЫМ границам
    for col in columns:
        fields.append({
            "field": col,
            "title": col,
            "paint": True,
            "summary": "sum",
            "paint_min": global_min,    # ← глобальные границы
            "paint_max": global_max,
        })

    # 'всего' — красим по СВОИМ колоночным min/max (оверрайды НЕ ставим)
    fields.append({
        "field": "всего",
        "title": "Grand Total",
        "paint": True,
        "summary": "sum",
        # без paint_min/paint_max — BasicTable возьмёт свои min/max для этой колонки
    })
...
def report_pdf_gtm(context, gtm_fig1, gtm_fig2, gtm_fig3, gtm_fig4, gtm_fig5, gtm_fig6, gtm_fig7, gtm_fig8, gtm_fig9, gtm_fig10, gtm_fig11, gtm_fig12, gtm_fig13, gtm_fig14):
    report = MailReport()
    ...
    gtm_p1 += report.add_block_header("Блок звонков по продажам")
    gtm_p1 += report.add_module_header("Статистика исходящих звонков")
    gtm_p1 += report.add_content_to_statistic(gtm_fig2)

    gtm_p1 += report.add_module_header("Причины недозвонов")
    gtm_p1 += report.add_content_to_statistic(gtm_fig3)

    gtm_p1 += report.add_block_header("Блок продаж")
    gtm_p1 += report.add_module_header("Конверсия подключений")
    gtm_p1 += report.add_figure(gtm_fig4)

    gtm_p1 += report.add_module_header("Статусы подключений")
    gtm_p1 += report.add_figure(gtm_fig5)

    gtm_p1 += report.add_module_header("Матрица подключений. Строки - Тариф абонента до продажи, Столбцы - Новый тариф")
    gtm_p1 += report.add_module_child_header("Подключения за последние 30 дней")
    gtm_p1 += report.add_content_to_statistic(gtm_fig6)

    gtm_p1 += report.add_module_child_header(f"Подключения за {report_date.strftime('%d.%m.%Y')}")
    gtm_p1 += report.add_content_to_statistic(gtm_fig7)

    gtm_p1 += report.add_module_header("Детальная статистика по дозвонам и подключениям за последние 30 дней")
    gtm_p1 += report.add_content_to_statistic(gtm_fig8)

    gtm_p1 += report.add_module_header("Разбивка продаж по ошибкам")
    gtm_p1 += report.add_content_to_statistic(gtm_fig9)
