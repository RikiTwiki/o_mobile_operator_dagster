# schedules/aggregate/csf_daily_statistics_backfill_schedule.py
...
START_DATE = date(2025, 12, 5)   # стартуем отсюда (первой будет 2025-12-04)
STOP_DATE  = date(2025, 11, 12)  # вниз до этой даты (включительно)

# имя финального отчёта можно переопределить через ENV
FINAL_FILE = os.getenv("CHAT_REPORT_FILE", "csf_daily_statistics.pdf")

def _reports_base() -> Path:
    # свой ENV + отдельная папка по умолчанию
    return Path(os.getenv("CHAT_REPORTS_BASE_DIR", "/opt/dagster/csf_reports"))
...
def _tags_fn(context):
    ...
    return {} if d is None else {
        ...
        "backfill": "csf_daily_statistics",
        "output_dir": str((_reports_base() / d.isoformat()).resolve()),
    }

csf_daily_statistics_backfill = ScheduleDefinition(
    name="csf_daily_statistics_backfill",
    job=csf_daily_statistics_job,
    cron_schedule="*/4 * * * *",     # каждые 15 минут
