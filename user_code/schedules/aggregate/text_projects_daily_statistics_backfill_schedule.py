# schedules/aggregate/text_projects_daily_statistics_backfill_schedule.py
...
START_DATE = date(2025, 12, 13)   # стартуем отсюда (первой будет 2025-12-04)
STOP_DATE = date(2025, 11, 12)  # включительно

FINAL_FILE = os.getenv("CHAT_REPORT_FILE", "text_projects_daily_statistics.pdf")

def _reports_base() -> Path:
    # В Docker по умолчанию: ./text_reports → /opt/dagster/text_reports
    return Path(os.getenv("REPORTS_BASE_DIR", "/opt/dagster/text_reports"))
...
def _tags_fn(context):
    ...
    return {} if d is None else {
        ...
        "backfill": "text_projects_daily_statistics",
        "output_dir": str((_reports_base() / d.isoformat()).resolve()),
    }

text_projects_daily_statistics_backfill = ScheduleDefinition(
    name="text_projects_daily_statistics_backfill",
