# schedules/aggregate/saima_daily_statistics_backfill_schedule.py
...
def _reports_base() -> Path:
    # В Docker по умолчанию: ./saima_reports → /opt/dagster/saima_reports
    return Path(os.getenv("REPORTS_BASE_DIR", "/opt/dagster/saima_reports"))
...
def _tags_fn(context):
    ...
    return {} if d is None else {
        ...
        "backfill": "saima_daily_statistics",
        "output_dir": str((_reports_base() / d.isoformat()).resolve()),
    }

saima_daily_statistics_backfill = ScheduleDefinition(
    name="saima_daily_statistics_backfill",
